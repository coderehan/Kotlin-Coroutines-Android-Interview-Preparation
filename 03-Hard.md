...
| | Channel | Flow |
|---|---|---|
| Style | hot, imperative (send/receive) | cold by default, declarative (operators) |
| Delivery | one item → exactly one receiver | one item → all collectors, if made hot |
...
fun listenToUpdates() = callbackFlow {
    ...
    awaitClose { api.unregister(listener) } // cleanup when the Flow is cancelled/closed
}
```
...
delay(500)
job.cancel() // stops the flow's collection at its next suspension point
```
